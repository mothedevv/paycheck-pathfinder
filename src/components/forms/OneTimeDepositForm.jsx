...
export default function OneTimeDepositForm({ deposit, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (deposit) {
        await base44.entities.OneTimeDeposit.update(deposit.id, formData);
      } else {
        await base44.entities.OneTimeDeposit.create(formData);
      }
      onSuccess();
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await base44.entities.OneTimeDeposit.delete(deposit.id);
      onSuccess();
    } catch (error) {
